...
## Project
`raggy` — a local RAG package (Python 3.10+, LangChain + Chroma + Ollama).
...
- Ollama service running with the models named in `config.yaml` (`nomic-embed-text`, `llama3.2`). Tests mock all Ollama/Chroma I/O — never require a live service in tests.
...
- `raggy/pipeline.py` — LCEL chain: retriever → `format_and_capture` (appends docs to `doc_sink`) → prompt → LLM.
